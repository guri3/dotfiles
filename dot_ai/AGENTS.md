...
- ヘッダーはh2から始めること
- コメントの末尾に `---` で区切った上で、利用しているAIサービス名を明記した一文を追記すること
  - 例: Claude Code（claude-sonnet-4-6）を利用している場合は「このコメントはClaude Code（claude-sonnet-4-6）によって生成されました。」
  - 例: Cursorを利用している場合は「このコメントはCursorによって生成されました。」
  - 利用しているサービス・モデルが不明な場合は「このコメントはAIによって生成されました。」とする
