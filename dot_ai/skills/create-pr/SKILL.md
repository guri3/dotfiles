...
PR作成（またはpush）後、CIの完了をバックグラウンドで監視し、結果を報告する。

- `gh pr checks <PR番号> --watch` をバックグラウンド実行で起動する
  - PR作成直後はchecksが未登録の場合がある。「no checks reported」等で即終了したときは30秒待ってから1回だけ再試行する
  - 再試行してもchecksが無い場合はCI未設定とみなし、監視せずその旨を一言伝えて終了する
- 監視はバックグラウンドに置いたまま、ユーザーへの応答や他の作業をブロックしない
- CIが完了したら結果を報告する
  - 全チェック成功: その旨とPRのURLを伝える
  - 失敗あり: 失敗したチェック名を列挙し、`gh run view <run_id> --log-failed` 等で失敗ログの要点を添える。原因の一次切り分けまでは行うが、修正はユーザーの指示を待つ
